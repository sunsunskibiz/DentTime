...
DentTime predicts appointment duration class for dental clinics (multi-class classification, target: `duration_class` ∈ {15, 30, 45, 60, 90, 105}). This spec covers the feature engineering pipeline that transforms the anonymized CSV into a model-ready feature matrix.
...
{
  "C_abc123": {
    "clinic_median_duration": 30.0,
    "clinic_pct_long": 0.12,
    "case_count": 450
  },
  "__global__": {
    "clinic_median_duration": 28.0,
    "clinic_pct_long": 0.09,
    "case_count": 255000
  }
}
```
...
Maps `scheduled_duration_min` → `duration_class` ∈ {15, 30, 45, 60, 90, 105}.

**Strategy: ceiling binning** — always round up to the nearest standard class. Rationale: DentTime's primary business risk is under-estimation (recommending too short a slot). Rounding down would tell a clinic to schedule less time than the treatment needs.
...
Covers non-standard durations: 10→15, 20→30, 40→45, 120→105.
Durations > 105 min are capped at 105 — document as known under-estimation risk for extreme cases.
...
- `duration_class` binning spot-checks: input 10→class 15, input 20→class 30, input 40→class 45, input 120→class 105
