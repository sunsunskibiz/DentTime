...
def _empty_state() -> Dict[str, Any]:
    return {"feature_psi": {}, "prediction_ratio": {}, "input_missing_rate": 0.0}
...
def main() -> None:
    # Start from the previous state so short-lived local dependency issues do not
    # erase already calculated metrics in monitoring/state.json.
    state: Dict[str, Any] = load_json(STATE_PATH) or _empty_state()
    state.setdefault("feature_psi", {})
    state.setdefault("prediction_ratio", {})
    state.setdefault("input_missing_rate", 0.0)

    try:
        reference = pd.read_parquet(REFERENCE_PATH) if REFERENCE_PATH.exists() else pd.DataFrame(columns=FEATURE_COLUMNS)
    except Exception as exc:
        print(f"Could not read reference parquet; PSI will keep previous values if present: {exc}")
        reference = pd.DataFrame(columns=FEATURE_COLUMNS)

    baseline_stats = load_json(FEATURE_STATS_PATH)
    live = _load_predictions()
    ...
    if "predicted_slot" in live.columns:
        class_ratio = live["predicted_slot"].dropna().astype(int).value_counts(normalize=True).sort_index()
    ...
    if baseline_stats and not live_features.empty:
        if "treatment_class" in live_features.columns:
            unknown_baseline = float(baseline_stats.get("treatment_class", {}).get("unknown_rate", 0.0))
            state["treatment_unknown_rate_baseline"] = unknown_baseline
        if "appt_hour_bucket" in live_features.columns:
    ...
    labeled = live.dropna(subset=["actual_slot"]).copy() if "actual_slot" in live.columns else pd.DataFrame()
