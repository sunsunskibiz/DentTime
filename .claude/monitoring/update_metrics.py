...
FALLBACK_REFERENCE_PATH = Path("features/features_train.parquet")
...
def main() -> None:
    ...
    reference_path = REFERENCE_PATH if REFERENCE_PATH.exists() else FALLBACK_REFERENCE_PATH
    ...
    if not live_features.empty:
        for col in FEATURE_COLUMNS:
            if not reference.empty and col in reference.columns and col in live_features.columns:
                state["feature_psi"][col] = psi_series(reference[col], live_features[col])
            elif col in live_features.columns:
                state["feature_psi"][col] = 0.0
